...
export const employeeUsers = async (_, args, context) => {
  try {
    const { filters } = args;
    ...
    const employeeIds = await Employees.findAll({
      attributes: ["personel_id", "adsoyad", "telefon", "gorev"],
    });
    ...
    await userIds.map(async (user) => {
      user.uye_id == 0 &&
        (await employeeIds.map(async (employee) => {
          if (user.personel == employee.personel_id) {
            employees.push({
